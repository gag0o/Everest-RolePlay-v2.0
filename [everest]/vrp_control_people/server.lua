...
function func.setPrisao2(id, tempo)
    if tempo > 0 then
        ...
        if player then
            ...
            vRP.addUserGroup(player,"ilegal")
        end
...
function func.addGroup()
	local source = source
	local user_id = vRP.getUserId(source)
	vRP.addUserGroup(user_id,"ilegal")
end
...
function func.isPolice()
    local source = source
    local user_id = vRP.getUserId(source)
    return vRP.hasPermission(user_id, "policia.permissao") or vRP.hasPermission(user_id, "juiz.permissao") or vRP.hasPermission(user_id, "admin.permissao")
end
