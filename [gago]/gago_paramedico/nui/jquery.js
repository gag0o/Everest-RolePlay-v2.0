...
$(function() {
    ...
    document.onkeyup = function(data) {
        if (data.which == 27) {
            $.post("http://gago_paramedico/mdtClose", JSON.stringify({}), function(datab) {});
        }
    };
});
...
const updateSearch = () => {
    $('#search-result').html(`
		...
		<div class="search-itens">
			...
			<div class="tickets">CONSULTAS</div>
			<div class="warnings">PRONTUARIOS</div>
		</div>
	`);
}

const infoUser = (user) => {
    $.post('http://gago_paramedico/infoUser', JSON.stringify({
        user
    }), (data) => {
        $('#info-result').html(`
			<div class="info-left">
			...
			<div class="info-right">
				<div class="parts-one"><r>Consultas:</r> ${data.tickets}</div> 
				<div class="parts-tree"><r>Prontuarios:</r> ${data.warnings}</div>
			</div>
		`);
    });
}
...
const arrestsUser = (user) => {
    $.post('http://gago_paramedico/arrestsUser', JSON.stringify({
        user
    }), (data) => {
        ...
        $('#info-result2').html(`
			<div class="sub-bar">
				...
				<div class="title-four">MEDICO</div>
			</div>
			<div class="itens">
				${itens.map((item) => (` < div class = "item" >
            < div class = "itens-one" > $ {
                item.data
            } < /div> < div class = "itens-two" > $ {
                item.value
            }
            meses < /div> < div class = "itens-tree" > $ {
                item.info
            } < /div> < div class = "itens-four" > $ {
                item.officer
            } < /div> < /div>
            `)).join('')}
			</div>
...
const ticketsUser = (user) => {
    $.post('http://gago_paramedico/ticketsUser', JSON.stringify({
        user
    }), (data) => {
        ...
        $('#info-result3').html(`
			<div class="sub-bar">
				...
				<div class="title-tree">SINTOMAS</div>
				<div class="title-four">MEDICO</div>
			</div>
			<div class="itens">
				${itens.map((item) => (` < div class = "item" >
            < div class = "itens-one" > $ {
                ...
                formatarNum(item.value)
            } < /div> < div class = "itens-tree" > $ {
                item.info
            } < /div> < div class = "itens-four" > $ {
                item.officer
            } < /div> < /div>
...
const warningsUser = (user) => {
    $.post('http://gago_paramedico/warningsUser', JSON.stringify({
        user
    }), (data) => {
        ...
        $('#info-result4').html(`
			<div class="sub-bar">
				...
				<div class="warning-title-two">SINTOMAS</div>
				<div class="warning-title-tree">MEDICO</div>
			</div>
			<div class="itens">
				${itens.map((item) => (` < div class = "item" >
            < div class = "warning-itens-one" > $ {
                item.data
            } < /div> < div class = "warning-itens-two" > $ {
                item.info
            } < /div> < div class = "warning-itens-tree" > $ {
                item.officer
            } < /div> < /div>
            `)).join('')}
			</div>
...
$(document).on('click', '.confirmar-warning', function() {
    ...
    if (user > 0) {
        $.post('http://gago_paramedico/warningUser', JSON.stringify({
            user,
            date,
            info,
            officer
...
$(document).on('click', '.confirmar-ticket', function() {
    ...
    if (user > 0) {
        $.post('http://gago_paramedico/ticketUser', JSON.stringify({
            user,
            value,
            date,
            info,
...
$(document).on('click', '.confirmar-arrest', function() {
    ...
    if (user > 0) {
        $.post('http://gago_paramedico/arrestUser', JSON.stringify({
            user,
            value,
            date,
            info,
