$(document).ready(function(){
	...
	document.onkeyup = function(data){
		if (data.which == 27){
			$.post("http://gago_comida/shopClose");
		}
	};
});
...
const utilidadesList = () => {
	$.post("http://gago_comida/utilidadesList",JSON.stringify({}),(data) => {
		let i = 0;
...
const bebidasList = () => {
	$.post("http://gago_comida/bebidasList",JSON.stringify({}),(data) => {
		let i = 0;
		const nameList = data.shopitens.sort((a,b) => (a.name > b.name) ? 1: -1);
		$('#inicio').html(`
			<input id="amount" class="qtd" maxlength="5" spellcheck="false" value="" placeholder="QUANT..">
...
$(document).on("click",".comprar",function(){
	...
	if($el && amount > 0){
		$.post("http://gago_comida/shopBuy",JSON.stringify({
