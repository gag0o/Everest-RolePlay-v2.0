	...
	while true do
		...
		if showHelp then
			if helpTimer > GetGameTimer() then
				showHelpText("Navegue com ~y~ W, A, S, D ~s~ e confirme com ~y~ ESPAÇO ~s~ para o bloco de código esquerdo.")
			elseif helpTimer > GetGameTimer()-helpCycle then
				showHelpText("Use as ~y~Setas~s~ e ~y~ENTER~s~ para o bloco de código correto")
			else
