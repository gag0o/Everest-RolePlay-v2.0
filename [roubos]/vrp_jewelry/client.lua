...
Citizen.CreateThread(function()
	while true do
		...
		if Vdist(coordenadaX,coordenadaY,coordenadaZ,GetEntityCoords(ped)) <= 1.0 and not andamento then
		...
		if andamento and tipo == 1 then
			drawTxt("~g~"..segundos.." SECONDS ~w~TO COMPLETE HACKING THE SECURITY CAMS",4,0.5,0.93,0.50,255,255,255,180)
...
Citizen.CreateThread(function()
	while true do
		...
		for _,v in pairs(locais) do
			if Vdist2(v.x,v.y,v.z,GetEntityCoords(ped)) <= 1.0 and not andamento then
				drawTxt("PRESS  ~b~E~w~  TO STEAL JEWELRY",4,0.5,0.93,0.50,255,255,255,180)
