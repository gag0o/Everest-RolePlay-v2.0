$(document).ready(function(){
	...
	document.onkeyup = function(data){
		if (data.which == 27){
			$.post("http://vrp_ebay/ebayClose",JSON.stringify({}),function(datab){});
		}
	};
});
...
const updateEbayList = () => {
	$.post("http://vrp_ebay/requestEbayList",JSON.stringify({}),(data) => {
		let i = 0;
...
const updateinvList = () => {
	$.post("http://vrp_ebay/requestinvList",JSON.stringify({}),(data) => {
		let i = 0;
...
$(document).on("click",".comprar",function(){
	let $el = $('.model.active');
	if($el){
		$.post("http://vrp_ebay/ebayBuy",JSON.stringify({
...
$(document).on("click",".retirar",function(){
	let $el = $('.model.active');
	if($el){
		$.post("http://vrp_ebay/ebayExtract",JSON.stringify({
			id: $el.attr('data-id-key'),
			userid: $el.attr('data-userid-key'),
			qtd: $el.attr('data-qtd-key'),
			itemid: $el.attr('data-itemid-key')
...
$(document).on("click",".vender",function(){
	...
	if($el && price > 0 && amount > 0){
		$.post("http://vrp_ebay/ebaySell",JSON.stringify({
			itemid: $el.attr('data-itemid-key'),
			amount,
			price,
			anony: anony.checked
