...
AddEventHandler('vrpdoorsystem:load', function(list) doors = list end)
    ...
    while true do
        ...
        for k, v in pairs(doors) do
            if GetDistanceBetweenCoords(x, y, z, v.x, v.y, v.z, true) <= 3 then
                local door = GetClosestObjectOfType(v.x, v.y, v.z, 1.0, v.hash,
                                                    false, false, false)
                if door ~= 0 then
                    ...
                    if v.lock == false then
                        ...
                        local lock, heading =
                            GetStateOfClosestDoorOfType(v.hash, v.x, v.y, v.z,
                                                        lock, heading)
