$(document).ready(function(){
	...
	window.addEventListener('message',function(event){
		...
		switch(item.action){
			case 'showMenu':
				updateGarages();
				actionContainer.slideDown(1000);
			break;

			case 'hideMenu':
				actionContainer.slideUp(1000);
			break;
		...
		$('#garagem').html(`
			...
			${nameList.map((item) => (`
				<div class="item" data-item-name="${item.name}">
					...
					<div class="item-text">
						<div class="item-left"><b>M</b> / <b>C</b> / <b>G:</b></div>
						<div class="item-right">${item.engine}% / ${item.body}% / ${item.fuel}%</div>
					</div>
					<div class="item-text">
