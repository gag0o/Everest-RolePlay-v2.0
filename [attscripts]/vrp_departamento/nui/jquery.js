$(document).ready(function(){
	...
	document.onkeyup = function(data){
		if (data.which == 27){
			$.post("http://vrp_departamento/shopClose");
		}
	};
});
...
const utilidadesList = () => {
	$.post("http://vrp_departamento/utilidadesList",JSON.stringify({}),(data) => {
		let i = 0;
...
const vestuarioList = () => {
	$.post("http://vrp_departamento/vestuarioList",JSON.stringify({}),(data) => {
		let i = 0;
		const nameList = data.shopitens.sort((a,b) => (a.name > b.name) ? 1: -1);
		$('#inicio').html(`
			<input id="amount" class="qtd" maxlength="7" spellcheck="false" value="" placeholder="QUANT..">
...
const bebidasList = () => {
	$.post("http://vrp_departamento/bebidasList",JSON.stringify({}),(data) => {
		let i = 0;
		const nameList = data.shopitens.sort((a,b) => (a.name > b.name) ? 1: -1);
		$('#inicio').html(`
			<input id="amount" class="qtd" maxlength="7" spellcheck="false" value="" placeholder="QUANT..">
...
$(document).on("click",".comprar",function(){
	...
	if($el && amount > 0){
		$.post("http://vrp_departamento/shopBuy",JSON.stringify({
