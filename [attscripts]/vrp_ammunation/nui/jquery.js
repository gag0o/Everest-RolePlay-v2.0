$(document).ready(function(){
	...
	document.onkeyup = function(data){
		if (data.which == 27){
			$.post("http://vrp_ammunation/shopClose");
		}
	};
});
...
const utilidadesList = () => {
	$.post("http://vrp_ammunation/utilidadesList",JSON.stringify({}),(data) => {
		let i = 0;
...
const armamentosList = () => {
	$.post("http://vrp_ammunation/armamentosList",JSON.stringify({}),(data) => {
		let i = 0;
		const nameList = data.shopitens.sort((a,b) => (a.name > b.name) ? 1: -1);
		$('#inicio').html(`
			<input id="amount" class="qtd" maxlength="7" spellcheck="false" value="" placeholder="QUANT..">
			<div class="comprar">COMPRAR</div>
			<div class="obs">Para <b>comprar</b> um item selecione-o abaixo, coloque a quantidade no menu acima e clique <b>comprar</b>.</div>
			<div class="title">ARMAMENTOS</div>
			${nameList.map((item) => (`
				<div class="model" data-index-key="${item.index}" data-price-key="${item.price}">
					<div class="id">${i = i + 1}</div>
					<div class="name">${item.name}</div>
...
const municoesList = () => {
	$.post("http://vrp_ammunation/municoesList",JSON.stringify({}),(data) => {
		let i = 0;
		const nameList = data.shopitens.sort((a,b) => (a.name > b.name) ? 1: -1);
		$('#inicio').html(`
			<input id="amount" class="qtd" maxlength="7" spellcheck="false" value="" placeholder="QUANT..">
...
$(document).on("click",".comprar",function(){
	...
	if($el && amount > 0){
		$.post("http://vrp_ammunation/shopBuy",JSON.stringify({
