$(document).ready(function() {
    ...
    $.post('http://kv_hud/getMinimapAnchor', JSON.stringify({}));
    ...
    window.addEventListener('message', function(event) {
        ...
        if (data.hunger != null) {
            $('#hunger').css('background', `linear-gradient(to top, rgba( 230, 115, 0, 0.5) ${data.hunger}%, rgba(0, 0, 0, 0.5) ${data.hunger}%)`);
        }
        
        if (data.thirst != null) {
            $('#thirst').css('background', `linear-gradient(to top, rgba( 49, 116, 225, 0.5) ${data.thirst}%, rgba(0, 0, 0, 0.5) ${data.thirst}%)`);
        }
